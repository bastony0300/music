...
const fs = require("fs");
const ms = require("ms")
exports.openBot = async (token, guildid, kind, owner,days) => {
    ...
    client.distube
        .on('playSong', (queue, song) => {
            ...
            var embed = new Discord.EmbedBuilder()
                ...
                .setFields({ name: "- User", value: `${song.member}` })
            ...
            const M = queue.textChannel.send({ embeds: [embed], content: `<a:1_:1189245342114394164> - 𝐏𝐋𝐀𝐘𝐈𝐍𝐆  **${song.name}** - \`${song.formattedDuration}\``, components: [row, row2] }).then((m) => {
    ...
    client.on("ready", async () => {
        ...
        client.user.setStatus("online")
            .client.user.setActivity({ "name": `𝐃𝐄𝐕 𝐁𝐘 𝐁𝐀𝐒𝐓𝐎𝐍𝐘`, "type": Discord.ActivityType.Playing })
        setInterval(async () => {
               ...
               console.log(client.days,client.DateNow);
                if((Date.now() - client.DateNow) >= ms(`${client.days}d`)) {
                    if(!user) {
                ...
                else if((Date.now() - client.DateNow) <= ms("12h")) {
                    if(!user) return;
